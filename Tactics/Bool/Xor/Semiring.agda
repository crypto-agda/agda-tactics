...
module Tactics.Bool.Xor.Semiring where

-- SECTION : importing the entire standard library and then some

open import Data.Bool renaming (_≟_ to _≟-Bool_; _xor_ to _⊕_)
...
open import Data.Nat.Properties using (m≤m⊔n; ⊔-⊓-0-isCommutativeSemiringWithoutOne)
...
{-
ocon : Bool → OpenPolynomial
ocon b = record { bound = 0; close = λ _ _ → con b }
...
mutual
  two-args : Bool → List (Arg Term) → AnswerM (OpenPolynomial × OpenPolynomial)
  two-args k [] = tko "no args, need 2" []
  two-args k (arg visible r a ∷ []) = ako "1 arg, need 2" [ arg visible r a ]
  two-args k (arg visible _ a ∷ arg visible _ b ∷ []) = mapA _,_ (interpret-expr k a) ⊛ interpret-expr false b
  two-args k as = ako "need 2 args, found something else" as

  interpret-expr : Bool → Term → AnswerM OpenPolynomial
  interpret-expr k (con c args) with c ≟-Name qt | c ≟-Name qf
  interpret-expr k (con c []) | yes _ | _     = pureA (ocon k)
  interpret-expr k (con c []) | _     | yes _ = pureA (ocon k)
  interpret-expr k (con c as) | _     | yes _ = tko "false with args" [ con c as ]
  interpret-expr k (con c as) | yes _ | _     = tko "true with args" [ con c as ]
  interpret-expr k (def f args) with f ≟-Name q+
  interpret-expr k (def f args) | yes _ = mapA (uncurry _o+_) (two-args k args)
  interpret-expr k (def f args) | no _ = carry-in k (mapA ovar (find-var (def f args)))
...
shuffle-test : (a b c : Bool) → a ⊕ b ⊕ true ⊕ c ⊕ b ≡ b ⊕ false ⊕ (c ⊕ a) ⊕ true ⊕ b
